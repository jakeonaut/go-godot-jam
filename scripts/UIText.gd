...
func _process(delta):
    if (global.numHerons > 0 or global.numBugs > 0 or global.numMushrooms > 0 or global.startTimer) and not global.is_in_cutscene:
        ...
        if global.nightfallTimer >= finale_timer:
            ...
            if global.startTimer:
                myText = myText + "nightfall: " + str(round_to_dec(finale_timer - global.nightfallTimer, 0))
            ...
            if global.numMushrooms > 0:
                myText = myText + "    shrooms: " + str(global.numMushrooms) + "/" + str(global.maxMushrooms)
            set_bbcode(myText)
    else:
